...
- **PRIMARY UI = brand Settings** (Kanwar 2026-07-12): the brand Settings tab gets a "Country tiers" section — tier rows (label, color, country multi-select) editable per brand. The agency-wide default set (workspace Settings) exists as the FALLBACK/template so 88 brands don't need hand-configuring — a brand shows the agency set until someone edits it there (which copies it into brand rows, then edits). Unassigned countries auto-bucket to "Other" — never dropped.
- **Tiers are a PLATFORM PRIMITIVE, not a report feature:** service `CountryTiers::resolve($brand)` (country→{tier_key,label,color} map) + the resolved map exposed in the brand detail API payload so EVERY surface can group by tier with zero extra queries. Wire it beyond the mom report in the same phase where cheap: Meta/TikTok/Google country breakdown views in the ads hub get a "Group by tier" toggle; dashboard country splits and the ads-audit report's country segments reuse the same resolver. One definition, fetched everywhere — never a second tier mapping anywhere in the codebase (add a comment on the service saying exactly that).
