...
final class CustomerMix
{
    ...
    /**
     * New-vs-returning counts for EVERY month in a range, in ONE ShopifyQL call
     * (customersByMonthRange groups by month) — used by S2's trailing
     * new/returning-sales trend so a multi-month graph costs a single request,
     * not one per month. Same honesty contract as forMonth(): null on no
     * connection / no scope / transport failure.
     *
     * @return array<string, array{customers: int, returning: int, new: int, orders: int}>|null
     *   keyed 'Y-m' (only months the store returned; a month with 0 customers is omitted)
     */
